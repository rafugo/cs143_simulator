...
\begin{description}
	...
	\item[$\bullet$ prepare\_host\_metrics(self)] This function uses the Flow rate dictionaries to construct per-host send/receive rates. 
...
Each Link object has two HalfLink objects associated, where each HalfLink object can only transmit information in a single direction. 
\begin{description}
...
\end{description}
\subsubsection{HalfLink Class}
\begin{description}
	\item[$\bullet$ add\_to\_buffer(self, Packet packet)] Tries to add the packet to the buffer. It is successful when there is enough space available in the buffer to accommodate it. Otherwise, the packet is dropped. It returns the amount of data that was added to the buffer in bits and 0 if the packet is dropped. 
	\item[$\bullet$ send\_packet(self)] This function tried to send a packet if it can. It manages removing packets from the buffer, keeping track of packets that are currently in transmission, ensuring packets arrive at their destination along this HalfLink on time, and keeping track of the link's rate.. 
	\item[$\bullet$ update\_link\_statistics(self)] This function records the HalfLink's buffer occupancy.
...
The window size for F1 starts out with exponential growth during the SS phase, until its window size grows too large, resulting in packet loss. This packet loss causes F1 to receive three duplicate acknowledgements, cutting its window size to half, and then timeout shortly thereafter, cutting its window size back to 1. After it times out, it re-enters slow start and then transitions to congestion avoidance, with linear window size growth when the window size reaches about 45 packets. 

The window size for F2 starts growing with exponential growth every other round trip time, but then it suddenly is cut in half because F2 receives three duplicate acknowledgements and enters the FRFR phase. This occurs because it drops a packet at L1 from H2 to H1 because its buffer fills with a combination of acknowledgments for F1 and packets for F2. Then F2 reenters SS and then transitions to the CA phase, in which it gradually grows and shrinks in size. 

From here on, approximately once every 3 seconds, F1 receives three duplicate acknowledgements, enters FRFR, times out, enters SS, then transitions back to CA. Whenever F1 enters FRFR and times out, its window size drops and stops transmitting packets for a while. In response, F2 observes a decrease in its RTT and increases its window size. 
...
We can see the relationship between window size and RTT here, where each significant dip in F2's round trip time is right before its window size starts growing again. 
...
Now, if we turn our attention to the buffer occupancy of L1 in the direction of H1 to H2, we see that the buffer's occupancy is directly related to the RTT of both flows. As the buffer fills, both flows' packets must spend a longer time sitting in the buffer, leading to an increased RTT for both flows. The buffer occupancy's drops correspond to each time F1 enters FRFR and stops transmitting packets for a while. 
...
L1's packet loss agrees with the buffer occupancy, as we see packets dropped when the buffer is full. Also these packet losses occur at about the same time as when F1 enters FRFR, which makes sense, as these losses are what trigger F1 to enter FRFR. 
...
The dips in L1's link rate correspond to the times F1 enters the FRFR phase and times out. It then increases back up to capacity as both F2's window size increases in response to F1 halting transmission of new packets and as F1's window size grows in the CA phase. 
...
The flow rates of F1 and F2 sum up to resemble L1's link rate. It's notable, however, that their sum exceeds the link rate, which is mainly due to the fact that we are only measuring link rate in one direction here. We also believe this is in part because of the way we are calculating the flow rates. 
